class PjaxAdapter extends Adapter {
  ...
  init($sidebar) {
    ...
    const pageChangeObserver = new window.MutationObserver(() => {
      // Trigger location change, can't just relayout as the sidebar might need to
    ...
    if (pjaxContainer) {
      ...
      function detectLocChange() {
        if (location.href !== href || location.hash !== hash) {
          ...
          // the extension does its own initialization after loading options.
          if (firstLoad) {
            firstLoad = false;
          } else {
            setTimeout(() => {
  ...
  _handlePjaxEvent(event, octotreeEventName, pjaxEventName) {
    ...
    try {
      ...
      // Only dispatch to native DOM if the event is started by the extension. If the event is started in the DOM, jQuery
      // wraps it in the originalEvent property, that's what we use to check.
  ...
  // @api protected
  _patchPjax() {
    // The pjax plugin ($.pjax) is loaded at the same time (document ready event) and
    // we don't know when $.pjax fully loaded, so we will do patching once in runtime
    if (!!this._$pjaxPatched) return;

    /**
     * Patch to prevent page refresh when selecting files in the sidebar.
     * The pjax library checks layout version which can cause unwanted refreshes.
     */
